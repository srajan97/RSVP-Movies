...
(
           ...
           GROUP BY   languages ), ger AS
(
       SELECT Sum(total_votes) AS 'German Votes'
       FROM   ger_ita
       WHERE  (german='German') ), ita AS
(
       SELECT Sum(total_votes) AS 'Italian Votes'
       FROM   ger_ita
       WHERE  (italian='Italian') )
SELECT *
FROM   ita
JOIN   ger;
...
-- Converting WGI in single currency


WITH top_genre AS
(
         SELECT   genre AS top_g ,
                  Count(movie_id)
         FROM     genre
         GROUP BY genre
         ORDER BY Count(movie_id) DESC limit 3 ), group_genre AS
(
         SELECT   movie_id,
                  Group_concat(genre) AS g_genre
         FROM     genre
         GROUP BY movie_id ), c_wgi AS
(
         SELECT   id,
                  title,
         ...
         FROM     movie
         ORDER BY c_worldwide_gross_income DESC ), final AS
(
                SELECT DISTINCT gg.movie_id,
                                gg.g_genre,
                                m.year,
                                m.title,
                                cwgi.c_worldwide_gross_income,
                                dense_rank() OVER (partition BY m.year ORDER BY cwgi.c_worldwide_gross_income DESC) AS movie_rank
                FROM            group_genre gg
                LEFT JOIN       movie m
                ON              gg.movie_id=m.id
                INNER JOIN      c_wgi cwgi
                ON              m.id=cwgi.id
                INNER JOIN      genre g
                ON              m.id = g.movie_id
                WHERE           g.genre IN
                                (
                                       SELECT top_g
                                       FROM   top_genre) )
SELECT g_genre AS 'Genre',
       title,
       year,
       c_worldwide_gross_income,
       movie_rank
FROM   final
WHERE  movie_rank<=5;
...
WITH act_drama
     AS (SELECT DISTINCT m.id,
                         NAME AS actress_name
         FROM   names n
                RIGHT JOIN role_mapping rm
                        ON n.id = rm.name_id
                RIGHT JOIN movie m
                        ON rm.movie_id = m.id
                INNER JOIN genre g
     ...
     AS (SELECT actress_name,
                ...
                Dense_rank ()
                  OVER (
                    ORDER BY Count(ad.id) DESC, Round(Avg(avg_rating), 2) DESC )
                AS
                   actress_rank
         FROM   act_drama ad
                INNER JOIN s_hit_movies shm
                        ON ad.id = shm.id
